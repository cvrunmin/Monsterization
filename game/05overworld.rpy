...
init -1 python:
    ...
    config.layers = ["submaster", "mapEngine"] + config.layers[:x] + ["npcPortraits"] + config.layers[x:]
    ...
    class OverworldDisplayable(renpy.Displayable):
        class Enemy(object):
            ...
            def __reduce__(self):
                ...
                return (_NestedClassGetter(),
                    (OverworldDisplayable, self.__class__.__name__, ),
                    state,
                    )
            def getRange(self):
        ...
        class Follower(Enemy):
            def __init__(self, x, y, range, roaming, sprites, facing, battleLabel):
                ...
                self.actionChoices = NonUniformRandom( [('walk', 47)] )
                self.goToLabel = None #share battlelabel var with enemy, but use it for action instead
                self.actionLabel = None #share battlelabel var with enemy, but use it for action instead
                self.chasing = True
            def __reduce__(self):
                state = self.__dict__.copy()
                return (_NestedClassGetter(),
            ...
            def move(self):
                ...
                if self.destX is not None and self.destY is not None: #Do we have a destination?
                    if not self.Walking:
                        self.Walking = True
                    if self.destX == self.rect.x and self.destY == self.rect.y:#Are we there?
                        if self.chasing: #if we're chasing something, chances are our destination is just the next step on a path, so handle that differently
                            if len(self.pathfinder.solution) > 0: #if we have a path to chase
            ...
            def update(self,obstacles,player):
                ...
                if self.Walking:
                    ...
                    self.image = Image(self.frames[0])
                if not self.rect.colliderect(player.rect): #if enemy is chasing and hasn't reached player yet
                    self.Walking = True #make sure they're walking
                else:
                    self.stop()
                if not self.posLocked:
        ...
        class Scenery(object):
            ...
            def __reduce__(self):
                state = self.__dict__.copy()
                return (_NestedClassGetter(),(OverworldDisplayable, self.__class__.__name__, ),state,)
        ...
        def __init__(self, map_layout = None, tileList = None, portals = None, portal_tiles = None, enemy_layout =None, enemySprites = None, NPCSprites = None, FollowerSprites = None, groundLayout = None, playerSprites = None, playerX = None, playerY = None, scrolling = True):
                    ...
                    for col in row:
                        for tile in enemySprites:
                            if col == tile[0] and not tile[6].won:
                                ...
                                enemyCount+= 1
                        for ff in FollowerSprites:
                            if col == ff[0]:
                                self.followerList.append(OverworldDisplayable.Follower(x, y, ff[1], ff[2], ff[3], ff[4], ff[5]))
                                self.zList.append(self.followerList[FollowerCount])
                                self.NPCObstacles.append(self.followerList[FollowerCount])
                                FollowerCount+= 1
                        for n in NPCSprites:
                            if col == n[0]:
        ...
        def render(self, width, height, st, at):
            ...
            for ff in self.followerList:
                if not self.talking:
                    ff.update(self.NPCObstacles, self.player)

            touch_interactive_scenery = False
            ...
            for s in self.sceneryList: #handle collision with important buildings
                ...
                if hasattr(s, 'actionLabel') and s.actionLabel is not None:
                    if abs(self.player.rect.centery - s.rect.centery) <= ((s.rect.height +self.player.rect.height)/2) and abs(self.player.rect.centerx - s.rect.centerx) <= ((s.rect.width +self.player.rect.width)/2):
                        touch_interactive_scenery = True
            ...
            if not self.talking and touch_interactive_scenery and renpy.get_screen("tips") is None:
                sname = "tips".split()
                scr = get_screen_variant(sname[0])
                if scr is not None:
                    _layer = get_screen_layer(sname)
                    _tag = scr.tag
                    scope = { }

                    if scr.parameters:
                        scope["_kwargs" ] = {}
                        scope["_args"] = ["按Space bar鍵進行互動"]
                    else:
                        scope.update({})

                    d = ScreenDisplayable(scr, _tag, _layer, {}, scope, transient=False)
                    ret.blit(renpy.render(d,width,height,st,at),(0,0))
                #renpy.invoke_in_new_context(lambda:renpy.show_screen("tips", "按Space bar鍵進行互動"))
                #renpy.show_screen("tips", "按Space bar鍵進行互動")
                #renpy.call_in_new_context("try_interactive_tips", True)
        ...
        def talk(self,vil):
            if hasattr(vil, 'actionLabel') and  vil.actionLabel is not None:
                ...
                if abs(self.player.rect.centery - vil.rect.centery) <= ((vil.rect.height +self.player.rect.height)/2) and abs(self.player.rect.centerx - vil.rect.centerx) <= ((vil.rect.width +self.player.rect.width)/2):
                    ...
                    self.talking = True
                    if False and vil.__class__ == OverworldDisplayable.Scenery: #if we're examining a building, we must be facing the front to interact.
                        if self.player.facing == 'up' and self.player.rect.top >= vil.rect.bottom: #TODO: cvrunmin cancel limitation
                            renpy.call_in_new_context(vil.actionLabel)
                    else:
                        results = renpy.call_in_new_context(vil.actionLabel) #NPC's will face you, so just call label.
                        if results is not None and len(results) >= 2:
                            if results[0] and results[1] is not None:
                                self.goTo = results[1]
                                renpy.timeout = 0
